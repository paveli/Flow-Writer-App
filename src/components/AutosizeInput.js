/* 
[License]
This source code is part of react-input-autosize 
https://github.com/JedWatson/react-input-autosize 
packege  destributed under the MIT license.

[Changes]
- All <div> elements were changed top <span> elements in component render() method.
- Quick fix added to disable outline on <input> element. Search file for "Quickfix". 
*/
...
const copyStyles = (styles, node) => {
	...
	node.style.outline = styles.outlineStyle;
	node.style.outlineStyle = styles.outline;
	node.style.outlineWidth = styles.outlineWidth;
...
class AutosizeInput extends Component {
	...
	render() {
		...
		if (!wrapperStyle.display) wrapperStyle.display = "inline";

		const inputStyle = {
			boxSizing: "content-box",
			width: `${this.state.inputWidth}px`,
			// Quickfix to the component for Murmur Writer project.
			// We need to disable outline and border.
			outline: "none",
			border: "0",
			margin: "0",
			padding: "0",
		...
		return (
			<span className={this.props.className} style={wrapperStyle}>
				...
				{this.props.placeholder ? (
					<span ref={this.placeHolderSizerRef} style={sizerStyle}>
						{this.props.placeholder}
					</span>
				) : null}
			</span>
